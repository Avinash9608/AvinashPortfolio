  ...
  React.useEffect(() => {
    if (inView) {
      ...
      const counter = setInterval(() => {
        currentCount += step;
        if (currentCount >= project.progress) {
          setCount(project.progress);
...
const ProjectCard = ({ project }: { project: typeof projects[0] }) => (
  <Card className="group relative overflow-hidden rounded-lg shadow-lg hover:shadow-xl transition-shadow duration-300">
    <div className="relative">
      ...
      />
      <div className="absolute inset-0 bg-black/20 dark:bg-black/50 bg-gradient-to-t from-black/60 dark:from-black/80 via-transparent to-black/10 dark:to-black/20" />
    </div>
    
    <div className="absolute inset-0 flex flex-col justify-end p-4 md:p-6">
      <CardTitle className="font-headline text-xl md:text-2xl text-white drop-shadow-lg mb-2">{project.title}</CardTitle>
      <div className="flex flex-wrap gap-2 mb-4">
        {project.tags.map(tag => <Badge key={tag} variant="secondary" className="backdrop-blur-sm bg-background/70 dark:bg-secondary/70">{tag}</Badge>)}
      </div>

      <div className="opacity-0 group-hover:opacity-100 transition-opacity duration-300">
        <p className="text-sm text-white dark:text-gray-200 mb-4 drop-shadow-md">{project.description}</p>
        <div className="flex justify-start gap-4">
          <Link href={project.github} target="_blank">
            <Button variant="secondary" size="sm" disabled={project.github === '#'}>
